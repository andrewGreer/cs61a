...
; expect (lambda (x y z) (* x x) (* y y) (* z z))
...
(define (fn y) (* y 5))
(define (lam x y) (/ (* x y) (fn y)))
(lam 32 26)
; expect 6.4

(define tobe (and 1 (= 1 2)))
(let ((x 17) (y 23) (fct (if (or tobe (> 1 0)) + -)))
     (fct x y))
; expect 40
...
(define (min s)
  (define (minimum s mini)
    (cond ((null? s) mini)
          ((< (car s) mini) (minimum (cdr s) (car s)))
          (else (minimum (cdr s) mini))))
  (minimum (cdr s) (car s)))
(define (two_of_three a b c)
  (- (+ (* a a) (* b b) (* c c)) (* (min (list a b c)) (min (list a b c)))))
(two_of_three 1 2 3)
; expect 13
(two_of_three 5 3 2)
; expect 34
...
(define (hailstone n)
  (define (stone n index)
    (cond ((= n 1) (and (print n) index))
          ((even? n) (and (print n) (stone (quotient n 2) (+ index 1))))
          (else (and (print n) (stone (+ (* n 3) 1) (+ index 1))))))
  (stone n 1))
(define a (hailstone 10))
a
; expect 7
...
(define (product n term)
  (define (iter n term total)
    (if (= n 1) total
        (iter (- n 1) term (* total (term n)))))
  (iter n term 1))
(product 3 identity)
; expect 6
(product 3 square)
