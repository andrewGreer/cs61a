...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while symbols is not nil:
            child.define(symbols.first, values.first)
            symbols, values = symbols.second, values.second
        # END PROBLEM 11
        return child
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        operands = scheme_apply(self, operands, env)
        operands = complete_eval(operands)
        return scheme_eval(operands, env, True)
        # END Problem 21
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):                 # effected by removing True
...
def do_and_form(expressions, env):
    ...
    while expressions.second is not nil:
        evaluated_value = scheme_eval(expressions.first, env)
        if not scheme_truep(evaluated_value):
            return evaluated_value
        expressions = expressions.second
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    while expressions.second is not nil:
        evaluated_value = scheme_eval(expressions.first, env)
        if scheme_truep(evaluated_value):
            return evaluated_value
        expressions = expressions.second
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 13
