...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 11
        return self.container == True and other.container == False and self.ant is None
        # END Problem 11
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while not random_or_none(start.bees):
            if start.entrance is end:
        ...
        # position = 0
        # if self.min_range:
        #     for x in range(0, self.min_range+1):
        #         start = start.entrance
        # while random_or_none(start.bees) == None and start != None:
        #     if self.max_range == position:
        #         return None
        #     start = start.entrance
        #     if start == hive:
        #         return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4
        bees_in_location = self.place.bees[:]
        if self.armor - amount <= 0:
            for bee in bees_in_location:
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        if QueenAnt.real_queen is True:
            self.real_queen = True
            QueenAnt.real_queen = False
        self.ants = []       # A list of Ants
        Ant.__init__(self)
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.real_queen:
            ...
            while end.exit is not None:
                if end.ant is not None:
                    if end.ant not in self.ants: # checking if the ant is already in the list
                        if end.ant.container == True:
                            end.ant.damage *= 2              # container update damage
                            self.ants.append(end.ant)
                            if end.ant.ant is not None and end.ant.ant not in self.ants:      # is there an ant contained and is it in the list?
                                end.ant.ant.damage *= 2
                                self.ants.append(end.ant.ant)
                        else:
                            end.ant.damage *= 2
                            self.ants.append(end.ant)
                    elif end.ant.container == True:
                        if end.ant.ant not in self.ants:
                            end.ant.ant.damage *= 2
                            self.ants.append(end.ant.ant)
                end = end.exit
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    old = bee.action
    new = effect(old)
    def long_lasting(colony):
        nonlocal duration
        if duration > 0:
            duration -= 1
            return new(colony)
        return old(colony)
    bee.action = long_lasting
    return long_lasting
    # END Problem EC
